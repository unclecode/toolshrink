 ...
 */
export function omissionMarker(chars, lines) {
    const parts = [`${chars.toLocaleString('en-US')} characters`];
    if (lines !== undefined && lines > 0)
        parts.push(`${lines.toLocaleString('en-US')} lines`);
 ...
 */
export function sliceSafely(text, limit) {
    if (limit <= 0)
        return '';
...
export function capLine(line, maxChars) {
...
export function capLines(text, maxChars) {
    if (maxChars <= 0)
        return text;
 ...
 */
export function takeHeadLines(lines, maxChars) {
    const kept = [];
    let used = 0;
...
export function takeTailLines(lines, maxChars) {
    const kept = [];
    let used = 0;
