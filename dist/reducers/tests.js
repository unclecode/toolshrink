...
export function detectTestOutput(text, hint) {
    const command = hint.command?.toLowerCase() ?? '';
    const commandLooksLikeTests = /\b(?:test|pytest|jest|vitest|mocha|gotest|cargo\s+test|go\s+test|npm\s+t\b|rspec|phpunit)/.test(command);
    ...
    for (const line of sample) {
        if (matches(SUMMARY_LINE, line))
            summaries++;
        else if (matches(PASS_LINE, line))
            passes++;
        else if (matches(FAIL_LINE, line))
            fails++;
    }
    // A summary line plus any test line is certain. Otherwise several test lines
    // are needed, because one stray "ok" proves nothing.
    if (summaries > 0 && passes + fails > 0)
        return true;
    if (commandLooksLikeTests && passes + fails >= 2)
        return true;
    return passes + fails >= 8;
}
export function reduceTestOutput(text, hint, budget) {
    ...
    if (keptCount === 0)
        return null; // nothing recognised, let size run
    if (keptCount === lines.length)
        return null; // nothing dropped, no gain
...
function joinWithMarkers(lines, keep) {
    const out = [];
    let dropChars = 0;
    let dropLines = 0;
    const flush = () => {
        if (dropLines > 0)
            out.push(omissionMarker(dropChars, dropLines));
        dropChars = 0;
        dropLines = 0;
    };
    for (let i = 0; i < lines.length; i++) {
