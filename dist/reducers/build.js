...
export function detectBuild(text, hint) {
    const command = hint.command?.toLowerCase() ?? '';
    const commandBuilds = /\b(?:tsc\b|cargo\s+(?:build|check)|go\s+build|make\b|gcc|clang|g\+\+|webpack|vite\s+build|esbuild|rustc)/.test(command);
    ...
    for (const line of sample) {
        if (matches(SUMMARY, line))
            summaries++;
        else if (matches(PROBLEM, line))
            problems++;
    }
    if (summaries > 0 && problems > 0)
        return true;
    if (commandBuilds && problems >= 1)
        return true;
    return problems >= 5;
}
export function reduceBuild(text, hint, budget) {
    const lines = splitLines(text);
