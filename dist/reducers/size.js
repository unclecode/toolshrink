 ...
 */
export function defaultKeep(hint) {
...
export function reduceBySize(text, hint, budget, keep = defaultKeep(hint)) {
...
function limitLines(lines, max, from) {
    if (lines.length <= max)
        return lines;
...
function finish(content, strategy, inputChars, inputLines, countedLines, keptLines) {
