...
const CONTEXT = 1;
export function detectDiff(text, hint) {
    ...
    if (headers >= 1 && changes >= 1)
        return true;
    return commandIsDiff && changes >= 2;
}
export function reduceDiff(text, hint, budget) {
    const lines = splitLines(text);
    const keep = new Array(lines.length).fill(false);
    const isStructure = (line) => STRUCTURE.some(pattern => pattern.test(line));
    ...
    const flush = () => {
        if (dropLines > 0)
            out.push(omissionMarker(dropChars, dropLines) + ' (unchanged)');
        dropChars = 0;
        dropLines = 0;
    };
    for (let i = 0; i < lines.length; i++) {
