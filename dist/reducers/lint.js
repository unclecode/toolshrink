...
const isFileHeader = (line) => /^[^\s].*\.\w+$/.test(line) || /^[^\s].*[/\\].+$/.test(line);
...
export function detectLint(text, hint) {
...
export function reduceLint(text, hint, budget) {
    ...
    for (const problem of problems) {
        ...
        if (list)
            ...
            byRule.set(problem.rule, [problem]);
    }
    ...
    for (const [rule, hits] of [...byRule.entries()].sort((a, b) => b[1].length - a[1].length)) {
        ...
        if (hits.length <= SUMMARISE_ABOVE) {
            for (const hit of hits)
                out.push(`${hit.level} ${rule}: ${hit.message}  (${hit.file}:${hit.where})`);
            continue;
        }
        out.push(`${first.level} ${rule} x${hits.length}: ${first.message}`);
        for (const hit of hits.slice(0, EXAMPLES))
            out.push(`    ${hit.file}:${hit.where}`);
    ...
    if (worst.length > 1) {
        ...
        for (const [file, count] of worst)
            out.push(`    ${count.toString().padStart(4)}  ${file}`);
    }
    const body = out.join('\n');
