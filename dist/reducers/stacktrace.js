...
const EDGE_FRAMES = 2;
const isFrame = (line) => FRAME.some(p => p.test(line));
const isForeign = (line) => FOREIGN.some(p => p.test(line));
export function detectStacktrace(text, _hint) {
    const sample = splitLines(text).slice(0, 400);
    const frames = sample.filter(isFrame).length;
    const hasHeader = sample.some(line => /Traceback \(most recent call last\)|^\w*(?:Error|Exception)[:\s]|^Caused by: /.test(line));
    // Enough frames to be worth touching; a 5-frame trace should stay whole.
    return hasHeader && frames >= 12;
}
export function reduceStacktrace(text, hint, budget) {
    ...
    for (let f = 0; f < lines.length; f++) {
        const line = lines[f];
        if (!(isFrame(line) && isForeign(line)))
            continue;
        keep[f] = false;
        let next = f + 1;
        while (next < lines.length && /^\s/.test(lines[next]) && !isFrame(lines[next])) {
            keep[next] = false;
            next++;
        }
    }
    ...
    while (i < lines.length) {
        ...
        for (let j = i; j < Math.min(i + EDGE_FRAMES, end); j++)
            keep[j] = true;
        for (let j = Math.max(end - EDGE_FRAMES, i + EDGE_FRAMES); j < end; j++)
            keep[j] = true;
        i = end;
    }
    ...
    const flush = () => {
        if (dropLines > 0)
            out.push(`    ${omissionMarker(dropChars, dropLines)} (dependency frames)`);
        dropChars = 0;
        dropLines = 0;
    };
...
export const stacktraceReducer = {
    name: 'stacktrace',
