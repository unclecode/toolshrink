...
export function detectInstall(text: string, hint: Hint): boolean {
  const command = hint.command?.toLowerCase() ?? ''
  const commandInstalls =
    /\b(?:npm\s+(?:i|install|ci)|pnpm\s+(?:i|install|add)|yarn\s+(?:add|install)|pip\s+install|cargo\s+(?:add|install|fetch)|poetry\s+(?:add|install)|bundle\s+install|go\s+get)\b/.test(command)
  ...
  if (commandInstalls && (noise + signal) >= 3) return true
  // Without a command hint, require the shape to be unmistakable.
  return noise >= 15 && noise >= sample.length * 0.4 && signal >= 1
}

export function reduceInstall(text: string, hint: Hint, budget: Budget): Reduction | null {
  const lines = splitLines(text)
  const keep = lines.map(line => matches(KEEP, line) && !matches(NOISE, line))

  const kept = keep.filter(Boolean).length
  if (kept === 0) return null
  if (kept === lines.length) return null

  const out: string[] = []
  let dropChars = 0
  let dropLines = 0
  const flush = () => {
    if (dropLines > 0) out.push(omissionMarker(dropChars, dropLines) + ' (progress)')
    dropChars = 0
    dropLines = 0
  }
