...
export function detectGitLog(text: string, hint: Hint): boolean {
  ...
  if (commandLogs && (full + oneline) >= 3) return true
...
export function reduceGitLog(text: string, hint: Hint, budget: Budget): Reduction | null {
  const lines = splitLines(text)
  ...
  if (isFull) {
    ...
    for (const line of lines) {
      if (ONELINE.test(line)) commits.push([line])
    }
  }

  if (commits.length <= RECENT + 2) return null
...
export const gitlogReducer: Reducer = {
  name: 'gitlog',
  detect: detectGitLog,
  reduce: reduceGitLog,
}
