...
export function detectLog(text: string, hint: Hint): boolean {
  ...
  if (shaped >= sample.length * 0.6) return true
  return pathIsLog && shaped >= sample.length * 0.2
}

export function reduceLog(text: string, hint: Hint, budget: Budget): Reduction | null {
  const lines = splitLines(text)
  const isTrouble = (line: string) => TROUBLE.some(pattern => pattern.test(line))

  const troubleCount = lines.filter(isTrouble).length
  // All quiet, or all noise: this cut has nothing to say. Size handles it.
  if (troubleCount === 0) return null
  if (troubleCount > lines.length * 0.5) return null

  const keep = new Array<boolean>(lines.length).fill(false)
  for (let i = 0; i < lines.length; i++) {
    if (!isTrouble(lines[i] as string)) continue
    for (let j = Math.max(0, i - BEFORE); j <= i; j++) keep[j] = true
  }
  for (let i = Math.max(0, lines.length - TAIL); i < lines.length; i++) keep[i] = true
  ...
  let dropLines = 0
  const flush = () => {
    if (dropLines > 0) out.push(omissionMarker(dropChars, dropLines) + ' (routine)')
    dropChars = 0
    dropLines = 0
  }
  for (let i = 0; i < lines.length; i++) {
