...
export function detectRepeat(text: string, _hint: Hint): boolean {
...
export function reduceRepeat(text: string, hint: Hint, budget: Budget): Reduction | null {
  ...
  while (i < lines.length) {
    const key = template(lines[i] as string)
    let end = i + 1
    while (end < lines.length && template(lines[end] as string) === key) end++
    const run = end - i

    if (run >= MIN_RUN) {
      for (let j = i; j < i + SHOW; j++) out.push(lines[j] as string)
...
export const repeatReducer: Reducer = {
  name: 'repeat',
  detect: detectRepeat,
  reduce: reduceRepeat,
}
