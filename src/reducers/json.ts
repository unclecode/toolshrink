...
export function detectJson(text: string, hint: Hint): boolean {
...
export function reduceJson(text: string, hint: Hint, budget: Budget): Reduction | null {
  let value: unknown
  try {
...
function sample(value: unknown, depth: number): unknown {
  if (depth > MAX_DEPTH) return typeMarker(value)

  if (Array.isArray(value)) {
    if (value.length <= KEEP_WHOLE) return value.map(item => sample(item, depth + 1))
  ...
  if (value !== null && typeof value === 'object') {
    ...
    if (entries.length <= KEEP_WHOLE_KEYS) {
      for (const [key, entry] of entries) out[key] = sample(entry, depth + 1)
      return out
    }
    for (const [key, entry] of entries.slice(0, SAMPLE_KEYS)) out[key] = sample(entry, depth + 1)
...
export const jsonReducer: Reducer = {
  name: 'json',
  detect: detectJson,
  reduce: reduceJson,
}
