...
export function detectTree(text: string, hint: Hint): boolean {
  ...
  if (pathLike >= sample.length * 0.8) return true
...
export function reduceTree(text: string, hint: Hint, budget: Budget): Reduction | null {
  ...
  for (const line of lines) {
    ...
    if (currentSection) {
      ;(groups.get(currentSection) as string[]).push(line)
      continue
    }
    ...
    ;(groups.get(parent) as string[]).push(line)
  }
  ...
  for (const [group, members] of groups) {
    if (isSectionHeader(group)) out.push(group)
  ...
  if (dropped === 0) return null // nothing collapsed, no gain
  const body = out.join('\n')
...
function isSectionHeader(group: string): boolean {
  return /:$/.test(group)
}
