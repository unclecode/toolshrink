...
function delimiterOf(lines: readonly string[]): string | null {
  ...
  for (const delimiter of [',', '\t', ';', '|']) {
    ...
    if (first >= 1 && counts.every(c => c === first)) return delimiter
  }
  return null
}

export function detectCsv(text: string, hint: Hint): boolean {
  const path = hint.path?.toLowerCase() ?? ''
  const lines = splitLines(text)
  if (lines.length < MIN_ROWS) return false
  const delimiter = delimiterOf(lines)
  if (!delimiter) return false
  if (path.endsWith('.csv') || path.endsWith('.tsv')) return true
  // Without a helpful path, require many consistent rows before claiming this.
  return lines.length >= 30
}

export function reduceCsv(text: string, hint: Hint, budget: Budget): Reduction | null {
  const lines = splitLines(text)
  const delimiter = delimiterOf(lines)
  if (!delimiter || lines.length < MIN_ROWS) return null
...
export const csvReducer: Reducer = {
  name: 'csv',
  detect: detectCsv,
  reduce: reduceCsv,
}
